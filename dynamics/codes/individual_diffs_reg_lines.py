...
slopes = list()
slopes.append(['PART_ID', f'{args.VAR}_Slope', f'{args.VAR}_Intercept'])
...
with tqdm(total=len(sacc_files)) as pbar:
    for part_id in sacc_files:  # for each participant
        ...
        tmp = df['AVG_RMx'].dropna().values
        X = df.index.values[:tmp.shape[0]].reshape(-1, 1)
        Y = tmp.reshape(-1, 1)
        if X.shape[0] < 1:
            slopes.append([part_id, 0, 0])
        else:
            linear_regressor = LinearRegression()
            linear_regressor.fit(X, Y)
            slopes.append([part_id, linear_regressor.coef_[0][0], linear_regressor.intercept_[0]])
...
dat = time.localtime()
slopes = pd.DataFrame(slopes[1:],columns=slopes[0])
slopes.to_csv(f'slopes_{args.VAR}.csv', index=False)
