...
#previously this was an used version, but due to high variance it's worth checking what happend when we split over median
...
# split over only two groups

wmc_band = ID_GF_WMC.WMC.quantile(1 / 2)
low_wmc = ID_GF_WMC[(ID_GF_WMC.WMC.between(-20, wmc_band))].PART_ID.tolist()
med_wmc = list() # for code consistency 
high_wmc = ID_GF_WMC[(ID_GF_WMC.WMC.between(wmc_band, 20))].PART_ID.tolist()
...
with tqdm(total=len(sacc_files)) as pbar:
    for part_id in sacc_files:  # for each participant
        ...
        part_wmc = ID_GF_WMC[ID_GF_WMC.PART_ID == int(part_id)]
        part_wmc = part_wmc.WMC.to_list()[0]
        print(part_wmc)
        ...
        if CONDITION in [CONDITIONS.LOW_WMC_FULL, CONDITIONS.WMC_LOW_LEV_EASY, CONDITIONS.WMC_LOW_LEV_MED,
                         ...
                         CONDITION.CORR_WMC_LOW_LEV_EASY, CONDITION.CORR_WMC_LOW_LEV_MED, CONDITION.CORR_WMC_LOW_LEV_HARD]:
        ...
        for idx in index:  # iterate only over correct trials
            ...
            beh_item = beh_data.loc[beh_idx] # zmiana ix na loc 
            ...
            if CONDITION in [CONDITIONS.CORR, CONDITIONS.LOW_WMC_CORR, CONDITIONS.MED_WMC_CORR,
                             CONDITIONS.HIGH_WMC_CORR, CONDITIONS.CORR_WMC_LOW_LEV_EASY, CONDITIONS.CORR_WMC_LOW_LEV_MED, CONDITIONS.CORR_WMC_LOW_LEV_HARD,
                             CONDITIONS.CORR_WMC_MED_LEV_EASY, CONDITIONS.CORR_WMC_MED_LEV_MED, CONDITIONS.CORR_WMC_MED_LEV_HARD, 
                             CONDITIONS.CORR_WMC_HIGH_LEV_EASY, CONDITIONS.CORR_WMC_HIGH_LEV_MED, CONDITIONS.CORR_WMC_HIGH_LEV_HARD]:
